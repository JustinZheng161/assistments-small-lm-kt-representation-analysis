...
The default inference configuration is CPU-compatible float32, batch size 32, and maximum sequence length 96. The default model revision is `main`, but each result records both the requested revision and the resolved Hugging Face commit hash. For numerical comparison, use the resolved commit hash printed in the result file rather than assuming that a mutable `main` revision will remain unchanged. Accelerator users may explicitly select, for example, `--device cuda --dtype bfloat16 --batch-size 64`. Results record these choices. They affect execution throughput and local logging, not the sampling rules, prompts, pooling definitions, classifier, score construction, or interpretation boundaries.
